...
export function Header({ user }: HeaderProps) {
  ...
  return (
    <header
      className={`bg-white border-b border-secondary-200 flex items-center justify-between px-4 md:px-6 sticky top-0 z-40 shadow-sm ${
    ...
    >
        ...
        {hasLogo ? (
          <div className="flex items-center shrink-0 mr-5 bg-transparent">
            <img
              src={logoUrl!}
              alt=""
              className="max-w-[80px] max-h-[70px] w-auto h-auto object-contain object-center"
            />
          </div>
        ) : (
          <div className="flex items-center justify-center shrink-0 w-[70px] h-[70px] text-primary-600 mr-5">
      ...
      <div className="flex items-center gap-4 min-w-0">
        <div className="flex items-center gap-3">
          <Avatar user={user} size="md" showName={false} />
