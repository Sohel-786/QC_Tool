...
export default function ReturnsPage() {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<ReturnForm>({
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-secondary-600 mb-1">
                          <span className="font-medium">Issue No:</span> {return_.issue?.issueNo}
                        </p>
                        {return_.remarks && (
        ...
        >
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <Select id="issueId" {...register('issueId', { valueAsNumber: true })} className="mt-1">
                <option value="">Select an issue</option>
                {activeIssues && Array.isArray(activeIssues) && activeIssues.map((issue) => (
                  <option key={issue.id} value={issue.id}>
                    {issue.issueNo} - {issue.tool?.toolName} ({issue.division?.name})
                  </option>
                ))}
              </Select>
              {errors.issueId && (
