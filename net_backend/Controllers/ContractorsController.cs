        ...
        public async Task<ActionResult<ApiResponse<object>>> Import(IFormFile file)
        {
            ...
            try
            {
                ...
                foreach (var row in result.Data)
                {
                    ...
                    {
                        result.Errors.Add(new RowError { Row = row.RowNumber, Message = $"Duplicate Contractor Name in file: {item.Name}" });
                        continue;
                    }

                    if (existingNames.Contains(nameLower))
                    {
                        result.Errors.Add(new RowError { Row = row.RowNumber, Message = $"Contractor '{item.Name}' already exists" });
                        processedInFile.Add(nameLower);
                        continue;
                    }

                    newItems.Add(new Contractor { Name = item.Name.Trim(), IsActive = true });
                    processedInFile.Add(nameLower);
                }

                if (newItems.Any()) { _context.Contractors.AddRange(newItems); await _context.SaveChangesAsync(); }
